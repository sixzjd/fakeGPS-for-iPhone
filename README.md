# FakeGPS v6.2.3 - iPhone 虚拟定位工具 | iOS GPS 模拟器
...
![macOS](https://img.shields.io/badge/macOS-FakeGPS.app-blue?logo=apple) ![Windows](https://img.shields.io/badge/Windows-FakeGPS.exe-blue?logo=windows) ![License](https://img.shields.io/badge/license-MIT-green)
...
| 平台 | 文件 | 说明 |
|------|------|------|
| macOS | `FakeGPS-macOS.dmg` | 打开后将 FakeGPS 拖入 Applications |
| Windows | `FakeGPS-Windows-Setup.exe` | 安装时可选择目标路径 |
...
### fakeGPS 是什么？
fakeGPS 是一款跨平台（macOS + Windows）iPhone 虚拟定位工具，可以通过 USB 将 iPhone 的 GPS 定位修改到世界上任意位置。免越狱、无需 Xcode，提供图形界面地图交互。
...
### 如何安装 fakeGPS？
推荐从 [GitHub Releases](https://github.com/sixzjd/fakeGPS-for-iPhone/releases/latest) 或官网 [fakegps.sixzjd.sbs](https://fakegps.sixzjd.sbs) 下载可执行文件，开箱即用；也可以克隆源码运行。
...
# FakeGPS v6.2.3 - iPhone Virtual Location Tool

FakeGPS is a cross-platform utility for setting a temporary virtual GPS location on an iPhone over USB. It runs on macOS and Windows, with no jailbreak or Xcode required.

### Highlights

- Set a location by clicking the map, searching a place, or entering coordinates
- Play GPX routes using the file's timestamps and speed data
- Convert between AMap GCJ-02 coordinates and GPS WGS-84 automatically
- Restore the iPhone's real location at any time
- Simple GUI with an interactive map

### Download

Get the latest release from [GitHub Releases](https://github.com/sixzjd/fakeGPS-for-iPhone/releases/latest) or [fakegps.sixzjd.sbs](https://fakegps.sixzjd.sbs).

| Platform | Package | How to use |
|----------|---------|------------|
| macOS | `FakeGPS-macOS.dmg` | Open it and drag FakeGPS into Applications |
| Windows | `FakeGPS-Windows-Setup.exe` | Choose the installation folder during setup |

No Python installation is required for the packaged applications.

### Requirements

- Connect the iPhone with a data-capable USB cable
- Trust the computer on the iPhone
- iOS 16 or later; iOS 17+ may ask for permission to start the tunneling service
- Windows users need [Apple Devices](https://apps.microsoft.com/store/detail/apple-devices/9NP83LWLPZ9K)
